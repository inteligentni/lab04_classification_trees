...
**Recall** (also known as *sensitivity*) calculates how many of the actual positives our model capture through labeling it as positive (True Positive). The formula is: 
...
Suppose we have a classifier for recognizing dogs in photographs. And in a picture containing 12 dogs and some cats, the classifier identifies 8 dogs (Fig. 2). Of the 8 identified as dogs, 5 actually are dogs (True Positives), while the rest are cats (False Positives). The program's precision is 5/8 while its recall is 5/12.

```{r fig.align='center', echo=FALSE, out.width = "400px", fig.cap="\\label{fig:figs}Precision and recall [1]"}
knitr::include_graphics('images/PrecisionRecall.png')
```

Two other important evaluation measures ar *Accuracy* and *F1-measure*.
...
[1] Precision and recall. (n.d.). Retrieved April 4, 2019, from https://en.wikipedia.org/wiki/Precision_and_recall